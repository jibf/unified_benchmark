...
def evaluate(args):
    ...
    for task_set in task_sets:
        if args.debug:
            specified_instructions.extend([task_messages[task_set][i] for i in [1,2,3]])
        else:
            if args.task_group == "All":
                specified_instructions.extend([task_messages[task_set][i] for i in range(len(task_messages[task_set]))])
            else:
                for i in range(len(task_messages[task_set])):
                    task_parameters = [task_messages[task_set][i]["Precise|Vague"], task_messages[task_set][i]["Completed|Error"], task_messages[task_set][i]["Single|Multiple_objects"], task_messages[task_set][i]["Single|Multiple_operations"], task_messages[task_set][i]["Structured/Unstructured"]]
    ...
    r = list(tqdm(pool1.imap(generator_partial, specified_instructions), total=len(specified_instructions)))
    pool1.close()
    pool1.join()
